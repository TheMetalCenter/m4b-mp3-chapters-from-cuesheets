...
- python3 (only for Scenario 5)
- findstr (should be pre-installed on windows, but if cmd doesn't recognize it you can find it in system32 and can copy it into working directory)
...
### Scenario 3: Creating single MP3 file with embedded chapters from randomly split mp3 files
	...
	8. Open up .cue file in text editor and change TITLES as desired for each chapter 
		- I recommend using macros on Notepad++ to quickly edit chapter titles
...
### Scenario 5: Creating single M4B file that retains chapers from two m4b files with embedded chapters (can repeat if more than two files)
